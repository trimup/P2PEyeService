...
public interface OrderInfoMapper {
    ...
    //根据项目查询已支付订单
    @Select("<script> select * from order_info where order_product_id in" +
    ...
    //根据项目id已支付订单
    @Select("<script> select * from order_info where order_product_id =#{order_product_id} " +
            "and (order_status_id=100565 OR order_status_id=100567 OR order_status_id=100617)  "+
            " </script>")
    public List<OrderInfo> selectOrderByPro(@Param("order_product_id") String order_product_id);
